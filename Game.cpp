...
void Game::UpdateGame()
{
	...
	if (!mActivated && mTimeSinceStart <= 0.0f) {
		mPlayer->SetState(ActorState::Active);
		mEnemy->SetState(ActorState::Active);
		mActivated = true;
		mChannel = Mix_PlayChannel(-1, GetSound("Assets/Sounds/Music.ogg"), -1);
	}
...
void Game::LoadData()
{
	...
	//MeshComponent* m = new MeshComponent(mTrack);
	mTrackMesh = new MeshComponent(mTrack);
	mTrackMesh->SetMesh(mRenderer->GetMesh("Assets/Track.gpmesh"));

	mMap = new HeightMap();
	mMap->LoadCSV("Assets/HeightMap/HeightMap.csv");
