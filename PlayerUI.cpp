...
{
	VehicleMove* p = mOwner->GetGame()->mPlayer->GetComponent<VehicleMove>();
	VehicleMove* e = mOwner->GetGame()->mEnemy->GetComponent<VehicleMove>();
	
	if (p->mCurrentLap > e->mCurrentLap || p->mCurrentLap == e->mCurrentLap && p->mLastCheckpoint > e->mLastCheckpoint
		|| p->mCurrentLap == e->mCurrentLap && p->mLastCheckpoint == e->mLastCheckpoint && 
		p->distToCheck() < e->distToCheck()) {
		return true;
	}
